...
namespace :database do
  ...
  task :wocheck => :environment do
    te           = WorkOrderProcess::BackgroundProcedure.new
    yesterday_wo = WorkOrder.where('created_at >= ? And created_at <= ? and check = ?', Time.now.at_beginning_of_day - 1.day,
                                   Time.now.at_beginning_of_day,2).limit(300)
  ...
  task :wofailcheck => :environment do
    te      = WorkOrderProcess::BackgroundProcedure.new
    fail_wo = WorkOrder.where('status = ? and created_at >= ? And created_at <= ?', 3, Time.now.at_beginning_of_day - 1.day,
